...
public func removePreferredWiFi(ssid: String) throws {
    let cleanSSID = ssid.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !cleanSSID.isEmpty else { return }
    guard let interface = wifiInterface() else {
        throw LidAwakeError("Wi-Fi interface was not found.")
    }

    let result = try run("/usr/sbin/networksetup", ["-removepreferredwirelessnetwork", interface, cleanSSID])
    guard result.status == 0 || result.output.contains("was not found") || result.error.contains("was not found") else {
        throw LidAwakeError(result.errorOrOutput(defaultMessage: "Could not remove Wi-Fi \(cleanSSID)."))
    }
}

public func disconnectCurrentWiFi() throws {
    guard let interface = wifiInterface() else {
        throw LidAwakeError("Wi-Fi interface was not found.")
    }

    let airportURL = URL(fileURLWithPath: "/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport")
    if FileManager.default.fileExists(atPath: airportURL.path) {
        let result = try run(airportURL.path, ["-z"])
        if result.status == 0 {
            return
        }
    }
